...
(async function checkAndUpserge() {
  try {
    ...
    const surgeCont =
      process.env.SURGE_CONTRACT ||
      `0xa15272c90f928da7c420c8b19e528b21ad634eb8`;
    ...
    const txnReceipt = await exponentialBackoff(async () => {
      ...
      return await upsurgeTxn.send({
        ...
        gas: new BigNumber(gas).times(2).toNumber(), // use twice as much gas to try and beat the other possible upsurgers
      });
    });
